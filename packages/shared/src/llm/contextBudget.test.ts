...
/**
 * 回归:CONTEXT_BREAKDOWN_META 曾漏掉 'pendingUser'(待发送),而它仍计入 usedTokens,
 * 导致弹窗图例各行之和 < 表头总数。图例必须覆盖每一个计入用量的分项。
 */
describe('context breakdown legend covers every counted component', () => {
  it('every breakdown key (incl. pendingUser) appears as a segment; segments sum to the total', () => {
    ...
    const segments = getContextBreakdownSegments(breakdown);
    const segKeys = segments.map((s) => s.key).sort();
    const allKeys = (Object.keys(breakdown) as (keyof ContextUsageBreakdown)[]).sort();
    expect(segKeys).toEqual(allKeys);
    // 图例各行之和必须等于表头用量总数,否则明细对不上(就是 pendingUser 漏列的 bug)。
    const segSum = segments.reduce((n, s) => n + s.tokens, 0);
    const total = Object.values(breakdown).reduce((n, v) => n + v, 0);
    expect(segSum).toBe(total);
    expect(segments.some((s) => s.key === 'pendingUser')).toBe(true);
  });
});
