...
| **R13** | 同 topic 两个 fresh 并发产生（race） | `acquireTopicSlot` 在事务内 `SELECT ... FOR UPDATE` 锁 topic key（pg_advisory_xact_lock(hashtext(topicKey))），保证同 topic 串行；TB1 / TB16 verify |
...
A: 不会。`acquireTopicSlot` 内部用 `pg_advisory_xact_lock(hashtext(topicKey))` 锁同 topic key，事务结束自动释放。第一个事务持锁时第二个 BLOCK；第一个 commit 完（创建 fresh run）第二个才进 SELECT，看到 active run → 走 merge 分支。TB1 / TB16 验证（用 `Promise.all([acquire1, acquire2])` 跑）。
