...
jest.mock('./dramaSaveStore', () => ({
  loadDramaSaveRaw: jest.fn(),
  saveDramaRaw: jest.fn(),
  clearDramaSave: jest.fn(),
}));
...
import { loadDramaSaveRaw } from './dramaSaveStore';
import { serializeSave } from './dramaSave';

const G = zh.games.drama;
const sayMock = api.dramaSay as jest.Mock;
const loadMock = loadDramaSaveRaw as jest.Mock;
...
beforeEach(() => {
  sayMock.mockReset();
  loadMock.mockReset().mockResolvedValue(null); // 默认无存档 → 正常开局、不弹续上
});
...
describe('DramaScreen 犬朝后宫(机制全开 + 选择有回响,驱动真实屏)', () => {
  it('行礼+结盟 → 说对 → 查对 → 点破(说台词)反将 → 进第二幕', async () => {
    ...
    expect(getByText(/第一幕 · 初入宫闱/)).toBeTruthy(); // 开场亮幕横幅
    ...
    fireEvent.press(getByTestId('say-opt-0')); // 多选题:挑对得体一句 → favor
    ...
    fireEvent.press(getByTestId('say-opt-0')); // 多选题:挑对点破一句 → triumph
    cont(getByText); // triumph 1
    cont(getByText); // triumph 2 → act2
    expect(getByText(/晋了嫔位/)).toBeTruthy(); // 进第二幕
    expect(getByText(/第二幕 · 御宴争辉/)).toBeTruthy(); // 跨幕亮新幕横幅
  });

  it('直言 + 谨慎 + 面圣说对 + 查错 → 冤案坏结局(有画面)', async () => {
    sayMock.mockResolvedValue({ ok: true, data: { pass: true, reply: 'REPLY_OK', score: 8 }, requestId: 'r' });
    const { getByText, getByTestId, findByText, queryByTestId } = mount();
    cont(getByText);
    cont(getByText);
    ...
    cont(getByText); // probe 引子 → 查案
    const culprit = generateCase(mulberry32(7), { count: 6, budget: 2 }).culpritIndex;
    fireEvent.press(getByTestId(`suspect-${(culprit + 1) % 6}`)); // 查错 → frame
    expect(getByText(/嘴角却悄悄翘/)).toBeTruthy(); // 冤案坏结局有画面
    expect(queryByTestId('drama-back-act')).toBeNull(); // 第一幕坏结局:检查点即起点,只给「从头再看」
  });

  it('当前说话的狗高亮、其余不高亮', () => {
    const { getByTestId, queryByTestId } = mount();
    expect(getByTestId('cast-speaking-laofu')).toBeTruthy(); // 开场老福在说
    expect(queryByTestId('cast-speaking-xuetuan')).toBeNull(); // 雪团这步没说话
  });

  it('有存档进屏 → 弹「继续上次」,点了跳到存档那一幕', async () => {
    const at2 = { sceneId: 'act2', stepIndex: 0, flags: ['polite', 'trust_molan'], status: 'playing' as const };
    loadMock.mockResolvedValue(serializeSave({ current: at2, checkpoint: at2 }));
    const { getByTestId, getByText, findByText } = mount();
    expect(await findByText(G.resumeTitle)).toBeTruthy();
    fireEvent.press(getByTestId('drama-resume'));
    expect(getByText(/晋了嫔位/)).toBeTruthy(); // 跳到第二幕开头
  });
});
