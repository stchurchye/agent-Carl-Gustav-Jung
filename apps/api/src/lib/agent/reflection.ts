...
/**
 * 累积发现渲染字节上限。v4 后单条 finding 可达 2000 字，且压缩是 best-effort
 * （test env / 超预算 / 无 LLM key / LLM 出错都跳过）→ completed 实际可达数万字，
 * 不能假设"恒被续跑期压缩门控住"。超预算时走 head+tail 截断（见 capFindingsHeadTail）。
 */
const REFLECT_FINDINGS_MAX_CHARS = 8000;

/**
 * 受字节预算约束地渲染发现行：超预算时**保留最早 + 最近**（中间略去）。
 * 收尾裁判最需要"最早的完成证据"（判目标是否早已达成）+ 最近进展，故两端都留。
 */
function capFindingsHeadTail(lines: string[], maxChars: number): string {
  ...
  for (const l of lines) {
    if (head.length > 0 && used + l.length + 1 > half) break;
    head.push(l);
    used += l.length + 1;
  }
  const tail: string[] = [];
  ...
  for (let i = lines.length - 1; i >= head.length; i--) {
    if (tail.length > 0 && used + lines[i].length + 1 > half) break;
    tail.unshift(lines[i]);
    used += lines[i].length + 1;
  }
  const omitted = lines.length - head.length - tail.length;
  if (omitted <= 0) return [...head, ...tail].join('\n');
  return [...head, `…（中间 ${omitted} 条已略）…`, ...tail].join('\n');
}
...
export function buildStepDigest(steps: AgentStep[], checkpoint?: AgentCheckpoint | null): string {
  // completed 受字节预算约束（head+tail）：既给收尾裁判最早的完成证据（round3 保留早期项的
  // 目的），又防长 run / 压缩失败时 completed 无界膨胀撑爆 reflection prompt（review over-size）。
  const accumulated =
    checkpoint && checkpoint.completed.length > 0
      ? '# 已确认的发现（累积）\n' +
        capFindingsHeadTail(
          checkpoint.completed.map((c) => `- ${c.text}: ${c.finding}`),
          REFLECT_FINDINGS_MAX_CHARS,
        ) +
        '\n\n# 最近步骤\n'
      : '';
  return (
