...
检索战法（研究/事实/文献类任务必读）：
- **中英双语查询**：同一主题安排中文和英文两路查询（外文文献和资料往往更全），
  例如「荣格 共时性」之外再加 "Jung synchronicity"。search_papers 用英文术语效果通常更好。
  search_web 支持 queries 数组——双语两路可在**一步**内扇出（如 queries: ["荣格 共时性", "Jung synchronicity"]），省步数预算。
- 来源语言不限——英文来源照样采用；最终回复由收尾层统一用中文转述，你不用管语言转换。
- 0 结果时先换关键词（同义词/更宽泛/另一语言），不要原样重试。
